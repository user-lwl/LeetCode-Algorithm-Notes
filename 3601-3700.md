...
class Solution {
    ...
    public long findLand(int x, int y) {
        if (ttGrid[x][y] == 0 || visited[x][y]) {
            return 0;
        }
        long result = ttGrid[x][y];
        visited[x][y] = true;
        if (x > 0) {
...
class Solution {
    public long maxProfit(int[] prices, int[] strategy, int k) {
        // 补0
        long[] s = new long[prices.length + 1];
        long[] p = new long[prices.length + 1];
        for (int i = 1; i < s.length; i++) {
            s[i] = s[i - 1] + prices[i - 1] * strategy[i - 1];
            p[i] = p[i - 1] + prices[i - 1];
        }
        long result = s[s.length - 1];
        for (int i = k; i < s.length; i++) {
            // 左  sum[i - k]
            // 右  sum[n] - sum[i]
            // 中  (p[i] - p[i - k / 2]) * 1
            long tempRes = s[i - k] + s[s.length - 1] - s[i] + p[i] - p[i - k / 2];
