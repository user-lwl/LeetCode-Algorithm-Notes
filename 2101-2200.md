...
class Solution {
    public int longestPalindrome(String[] words) {
        Map<String, Integer> noEquMap = new HashMap<>();
        Set<String> equSet = new HashSet<>();
        int result = 0;
        for (int i = 0; i < words.length; i++) {
            String thisStr = words[i];
            String otherStr = new String(new char[]{words[i].charAt(1), words[i].charAt(0)});
            if (thisStr.equals(otherStr)) {
                if (equSet.contains(thisStr)) {
                    result += 4;
                    equSet.remove(thisStr);
                } else {
                    equSet.add(thisStr);
                }
            } else {
                if (noEquMap.containsKey(otherStr)) {
                    int cnt = noEquMap.get(otherStr);
                    result += 4;
                    if (cnt == 1) {
                        noEquMap.remove(otherStr);
                    } else {
                        noEquMap.put(otherStr, cnt - 1);
                    }
                } else {
                    noEquMap.put(thisStr, noEquMap.getOrDefault(thisStr, 0) + 1);
                }
            }
        }
        return equSet.size() == 0 ? result : result + 2;
    }
}
```
...
class Solution {
    public int[] rearrangeArray(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i+=2) {
            nums[i] = zheng.get(indexZ++);
            nums[i + 1] = fu.get(indexF++);
        }
        return nums;
    }
}
...
class Solution {
    public List<Integer> findLonely(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            int cnt = tempMap.get(nums[i]);
            if (cnt == 1 && !tempMap.containsKey(nums[i] - 1) && !tempMap.containsKey(nums[i] + 1)) {
...
class Solution {
    public long smallestNumber(long num) {
        ...
        int minNumberNotZero = 10;
        while (num > 0) {
            int tempNumber = (int) (num % 10);
            num /= 10;
            number[tempNumber]++;
            if (tempNumber != 0) {
                minNumberNotZero = Math.min(minNumberNotZero, tempNumber);
            }
        }
        if (minNumberNotZero == 10) {
            minNumberNotZero = 0;
        }
        long result = 0;
        if (flag == 1) {
            result += minNumberNotZero;
            number[minNumberNotZero]--;
...
class Solution {
    public int minimumOperations(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            if (i % 2 == 0) {
                int key = nums[i];
                int value = ouMap.getOrDefault(key, 0) + 1;
                if (value > ouMaxCnt) {
                    if (key == ouMaxNum) {
                        ouMaxCnt = value;
                    } else {
                        ouSecondCnt = ouMaxCnt;
                        ouSecondNum = ouMaxNum;
                        ouMaxCnt = value;
                        ouMaxNum = key;
                    }
                } else if (value > ouSecondCnt) {
                    if (key == ouSecondNum) {
                        ouSecondCnt = value;
                    } else {
                        ouSecondCnt = value;
                ...
                int key = nums[i];
                int value = jiMap.getOrDefault(key, 0) + 1;
                if (value > jiMaxCnt) {
                    if (key == jiMaxNum) {
                        jiMaxCnt = value;
                    } else {
                        jiSecondCnt = jiMaxCnt;
                        jiSecondNum = jiMaxNum;
                        jiMaxCnt = value;
                        jiMaxNum = key;
                    }
                } else if (value > jiSecondCnt) {
                    if (key == jiSecondNum) {
                        jiSecondCnt = value;
                    } else {
                        jiSecondCnt = value;
        ...
        if (ouMaxNum == jiMaxNum) {
            return nums.length - Math.max(ouMaxCnt + jiSecondCnt, ouSecondCnt + jiMaxCnt);
        } else {
            return nums.length - ouMaxCnt - jiMaxCnt;
        }
    }
}
```
...
class Solution {
    public String repeatLimitedString(String s, int repeatLimit) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            int cnt = tempMap.getOrDefault(temp, 0);
            if (cnt == 0) {
                heap.add(temp);
            }
            tempMap.put(temp, cnt + 1);
        }
        StringBuilder sb = new StringBuilder();
        boolean isSecond = false;  // 是否使用第二个值
        while (!heap.isEmpty()) {
            if (!isSecond) {
                Character tmp = heap.poll();
                int cnt = tempMap.get(tmp);
                int need = Math.min(cnt, repeatLimit);
                cnt -= need;
                while (need > 0) {
                    sb.append(tmp);
                    need--;
...
class Solution {
    public int[] sortJumbled(int[] mapping, int[] nums) {
        String[][] sortList = new String[nums.length][2];
        for (int i = 0; i < nums.length; i++) {
            String numStr = String.valueOf(nums[i]);
            StringBuilder sb = new StringBuilder();
            for (int j = 0; j < numStr.length(); j++) {
                sb.append(mapping[numStr.charAt(j) - '0']);
            }
            sortList[i][0] = numStr;
            sortList[i][1] = sb.toString();
        }
        ...
        int[] result = new int[sortList.length];
        for (int i = 0; i < sortList.length; i++) {
            result[i] = Integer.valueOf(sortList[i][0]);
        }
        return result;
    }
}
...
class Solution {
    public long minimalKSum(int[] nums, int k) {
        ...
        tempNums[tempNums.length - 2] = 0;
        tempNums[tempNums.length - 1] = Integer.MAX_VALUE;
        Arrays.sort(tempNums);
        for (int i = 1; i < tempNums.length; i++) {
            int left = tempNums[i - 1];
            int right = tempNums[i];
            int cnt = right - left - 1;
            if (cnt > 0) {
                int startNumber = left + 1;
                int endNumber = Math.min(left + cnt, left + k);
                k -= endNumber - startNumber + 1;
                sum += ((long) startNumber + endNumber) * (endNumber - startNumber + 1) / 2;
            }
            if (k <= 0) {
                break;
            }
