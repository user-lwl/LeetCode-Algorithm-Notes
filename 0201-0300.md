...
class Solution {
    ...
    public void doHelper(int index, int sum) {
        if (tempList.size() == globalK && sum == globalN) {
            result.add(new ArrayList<>(tempList));
            return;
        }
        if (sum >= globalN || tempList.size() >= globalK) {
            return;
        }
...
class Solution {
    public int[] productExceptSelf(int[] nums) {
        int[] before = new int[nums.length + 2];
        ...
        for (int i = nums.length - 1; i >= 0; i--) {
            after[i + 1] = after[i + 2] * nums[i];
        }
        int[] result = new int[nums.length];
        for (int i = 0; i < result.length; i++) {
            result[i] = before[i] * after[i + 2];
        }
        return result;
    }
}
...
class Solution {
    ...
    public List<Integer> diffWaysToCompute(String expression) {
        global = expression.toCharArray();
        return dfs(0, global.length - 1);
    }
    public List<Integer> dfs(int left, int right) {
        List<Integer> result = new ArrayList<>();
        for (int i = left; i <= right; i++) {
            if (global[i] >= '0' && global[i] <= '9') {
                continue;
            }
...
class Solution {
    public void gameOfLife(int[][] board) {
        int[][] liveCeil = new int[board.length][board[0].length];
        ...
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[i].length; j++) {
                if (board[i][j] == 1) {
                    if (liveCeil[i][j] < 2 || liveCeil[i][j] > 3) {
                        board[i][j] = 0;
                    }
                } else {
                    if (liveCeil[i][j] == 3) {
