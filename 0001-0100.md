...
class Solution {
    ...
    List<List<Integer>> resultList = new ArrayList<>();
    int globalTarget;
    int[] golbalCandidates;

    public List<List<Integer>> combinationSum(int[] candidates, int target) {
        globalTarget = target;
        golbalCandidates = candidates;
        List<Integer> tempList = new ArrayList<>();
        doHelper(0, 0, tempList);
        return resultList;
    }
    public void doHelper(int index, int sum, List<Integer> tempList) {
        if (sum == globalTarget) {
        ...
        for (int i = index; i < golbalCandidates.length; i++) {
            tempList.add(golbalCandidates[i]);
            doHelper(i, sum + golbalCandidates[i], tempList);
...
class Solution {
    ...
    public void doHelper(int index, int sum, List<Integer> tempList) {
        ...
        for (int i = index + 1; i < globalCandidates.length; i++) {
            if (i > index + 1 && globalCandidates[i] == globalCandidates[i - 1]) {
                continue;
            }
            tempList.add(globalCandidates[i]);
            doHelper(i, sum + globalCandidates[i], tempList);
            tempList.remove(tempList.size() - 1);
        }
    }
}
    ...
    public String multiply(String num1, String num2) {
        ...
        for (int i = 0; i < num2.length(); i++) {
            ...
            for (int j = num1.length() - 1; j >= 0; j--) {
                int otherNumber = num1.charAt(j) - '0';
                int tempRes = number * otherNumber + next;
                sb.insert(0, tempRes % 10);
                next = tempRes / 10;
            }
            if (next != 0) {
                sb.insert(0, next);
        ...
        String result = tempString.get(0);
        for (int i = 1; i < tempString.size(); i++) {
            int hasNext = 0;
            String otherNumber = tempString.get(i);
            int resIndex = result.length() - 1;
            int otherIndex = otherNumber.length() - 1;
            StringBuilder sb = new StringBuilder();
            while (resIndex >= 0 && otherIndex >= 0) {
                int resN = result.charAt(resIndex) - '0';
                int otherN = otherNumber.charAt(otherIndex) - '0';
                int tempRes = resN + otherN + hasNext;
                sb.insert(0, tempRes % 10);
                hasNext = tempRes / 10;
                resIndex--;
                otherIndex--;
...
class Solution {
    ...
    List<Integer> globalNums = new ArrayList<>();
    List<List<Integer>> resultList = new ArrayList<>();

    public List<List<Integer>> permute(int[] nums) {
        for (int i = 0; i < nums.length; i++) {
            globalNums.add(nums[i]);
        }
        doHelper(0);
        return resultList;
    }
    public void doHelper(int index) {
        if (index == globalNums.size() - 1) {
            resultList.add(new ArrayList<>(globalNums));
            return;
        }
        for (int i = index; i < globalNums.size(); i++) {
            swap(index, i);
            doHelper(index + 1);
            swap(index, i);
        }
    ...
    public void swap(int indexA, int indexB) {
        int temp = globalNums.get(indexA);
        globalNums.set(indexA, globalNums.get(indexB));
        globalNums.set(indexB, temp);
    }
}
```
...
class Solution {
    ...
    public void doHelper(int index) {
        ...
        for (int i = index; i < globalNums.size(); i++) {
            if (tempSet.contains(globalNums.get(i))) {
                continue;
            }
            tempSet.add(globalNums.get(i));
            swap(i, index);
            doHelper(index + 1);
            swap(i, index);
        }
...
class Solution {
    public List<Integer> spiralOrder(int[][] matrix) {
        ...
        int[][] tempArray = new int[matrix.length + 2][matrix[0].length + 2];
...
class Solution {
    public String simplifyPath(String path) {
        ...
        for (int i = 0; i < splitStr.length; i++) {
            if (splitStr[i].length() == 0 || splitStr[i].equals(".")) {
                continue;
            } else if (splitStr[i].equals("..")) {
                pathQue.pollLast();
            } else {
                pathQue.addLast(splitStr[i]);
            }
        }
        StringBuilder sb = new StringBuilder();
        while (!pathQue.isEmpty()) {
            sb.append("/").append(pathQue.pollFirst());
        }
        if (sb.length() == 0) {
            sb.append("/");
        }
...
class Solution {
    ...
    public void doHelper(int index, List<Integer> tempList) {
        if (tempList.size() == globalK) {
            resultList.add(new ArrayList<>(tempList));
            return;
        }
        for (int i = index; i <= globalN; i++) {
            tempList.add(i);
            doHelper(i + 1, tempList);
            tempList.remove(tempList.size() - 1);
        }
    }
}
            ...
            for (int j = i + 1; j < s.length(); j++) {
                for (int k = j + 1; k < s.length(); k++) {
                    for (int m = k + 1; m < s.length(); m++) {
                        ...
                        String two = s.substring(j, k);
                        String three = s.substring(k, m);
                        String four = s.substring(m, s.length());
                        long n1 = Long.valueOf(one);
                        long n2 = Long.valueOf(two);
                        long n3 = Long.valueOf(three);
                        long n4 = Long.valueOf(four);
                        if ((one.charAt(0) == '0' && n1 != 0) || (two.charAt(0) == '0' && n2 != 0) || (three.charAt(0) == '0' && n3 != 0) || (four.charAt(0) == '0' && n4 != 0)) {
                            continue;
                        }
                        if ((n1 == 0 && one.length() > 1) || (n2 == 0 && two.length() > 1) || (n3 == 0 && three.length() > 1) || (n4 == 0 && four.length() > 1)) {
                            continue;
                        }
                        if (n1 >= 0 && n1 <= 255 && n2 >= 0 && n2 <= 255 && n3 >= 0 && n3 <= 255 && n4 >= 0 && n4 <= 255) {
                            resultList.add(n1 + "." + n2 + "." + n3 + "." + n4);
                        }
                    }
                }
            }
