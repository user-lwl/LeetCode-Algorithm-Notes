...
class Solution {
    public int[] countNumbers(int cnt) {
        ...
        endNumber = Integer.valueOf(temp);
...
class Solution {
    ...
    public void doFlip(TreeNode node) {
        if (node == null) {
            return;
        }
...
class Solution {
    ...
    public void putLevel(TreeNode node, Integer level) {
        if (node == null) {
            return;
        }
        ...
        List<Integer> temp = result.get(level - 1);
        temp.add(node.val);
        putLevel(node.left, level + 1);
        putLevel(node.right, level + 1);
    }
}
```
...
class Solution {
    public Node copyRandomList(Node head) {
        ...
        Node tempNode = head;
        Node tempNewNode = newHead;
        while (tempNode.next != null) {
            Node nextNode = tempNode.next;
            Node newNextNode = new Node(nextNode.val);
            tempNewNode.next = newNextNode;
            tempMap.put(nextNode, newNextNode);
            tempNode = nextNode;
            tempNewNode = newNextNode;
        }
        tempNode = head;
        tempNewNode = newHead;
        while (tempNode != null) {
            Node randomNode = tempNode.random;
            Node randomNewNode = tempMap.getOrDefault(randomNode, null);
            tempNewNode.random = randomNewNode;
...
class Solution {
    ...
    public int findTargetNode(TreeNode root, int cnt) {
        middleHelper(root);
        return tempList.get(tempList.size() - cnt);
    }
...
class Solution {
    public int trainingPlan(int[] actions) {
        Map<Integer, Integer> tempMap = new HashMap<>();
        for (int i = 0; i < actions.length; i++) {
            tempMap.put(actions[i], tempMap.getOrDefault(actions[i], 0) + 1);
        }
        int result = 0;
...
class Solution {
    ...
    public TreeNode getParentNode(TreeNode node, TreeNode p, TreeNode q) {
        if (node.val == p.val || node.val == q.val || (node.val - p.val) * (node.val - q.val) < 0) {
            return node;
        }
        if (node.val > p.val) {
            return getParentNode(node.left, p, q);
        } else {
            return getParentNode(node.right, p, q);
        }
    }
}
```
...
class Solution {
    ...
    public TreeNode getParentNode(TreeNode node, TreeNode p, TreeNode q) {
        ...
        TreeNode leftNode = getParentNode(node.left, p, q);
        TreeNode rightNode = getParentNode(node.right, p, q);
        if (leftNode != null && rightNode != null) {
            return node;
