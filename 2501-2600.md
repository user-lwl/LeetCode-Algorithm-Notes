# 2506.统计相似字符串对的数目

```java
class Solution {
    public int similarPairs(String[] words) {
        for (int i = 0; i < words.length; i++) {
            int[] number = new int[26];
            for (int j = 0; j < words[i].length(); j++) {
                number[words[i].charAt(j) - 'a']++;
            }
            StringBuilder sb = new StringBuilder();
            for (int j = 0; j < number.length; j++) {
                if (number[j] != 0) {
                    sb.append((char) (j + 'a'));
                }
            }
            words[i] = sb.toString();
        }
        int result = 0;
        for (int i = 0; i < words.length; i++) {
            for (int j = i + 1; j < words.length; j++) {
                if (words[i].equals(words[j])) {
                    result++;
                }
            }
        }
        return result;
    }
}
```

# 2511.最多可以摧毁的敌人城堡数目

```java
class Solution {
    public int captureForts(int[] forts) {
        int result = 0;
        for (int i = 0; i < forts.length; i++) {
            int temp = forts[i];
            int startIndex = i;
            if (temp == 1 || temp == -1) {
                i++;
                while (i < forts.length && forts[i] == 0) {
                    i++;
                }
                if (i == forts.length) {
                    break;
                }
                if (temp + forts[i] == 0) {
                    result = Math.max(result, i - startIndex - 1);
                }
                i--;
            }
        }
        return result;
    }
}
```



# 2515.到目标字符串的最短距离

```java
class Solution {
    public int closestTarget(String[] words, String target, int startIndex) {
        String[] wordAndWord = new String[words.length * 2];
        System.arraycopy(words, 0, wordAndWord, 0, words.length);
        System.arraycopy(words, 0, wordAndWord, words.length, words.length);
        int minIndex = -1;
        int maxIndex = -1;
        for (int i = startIndex; i < startIndex + words.length; i++) {
            if (wordAndWord[i].equals(target)) {
                if (minIndex == -1) {
                    minIndex = i;
                    maxIndex = i;
                } else {
                    maxIndex = i;
                }
            }
        }
        return minIndex == -1 ? minIndex : Math.min(minIndex - startIndex, words.length - maxIndex + startIndex);
    }
}
```

# 2520.统计能整除数字的位数

```java
class Solution {
    public int countDigits(int num) {
        int result = 0;
        int number = num;
        while (number > 0) {
            int temp = number % 10;
            if (num % temp == 0) {
                result++;
            }
            number /= 10;
        }
        return result;
    }
}
```

# 2525.根据规则将箱子分类

```java
class Solution {
    public String categorizeBox(int length, int width, int height, int mass) {
        boolean bulky = false;
        boolean heavy = false;
        long v = (long) length * width * height;
        if (length >= (int)1e4 || width >= (int)1e4 || height >= 1e4 || v >= (int)1e9) {
            bulky = true;
        }
        if (mass >= 100) {
            heavy = true;
        }
        if (bulky && heavy) {
            return "Both";
        } else if (bulky) {
            return "Bulky";
        } else if (heavy) {
            return "Heavy";
        } else {
            return "Neither";
        }
    }
}
```

# 2526.找到数据流中的连续整数

```java
class DataStream {

    List<Integer> tempList = new ArrayList<>();
    Map<Integer, Integer> tempMap = new HashMap<>();
    int ttVal;
    int ttk;

    public DataStream(int value, int k) {
        ttVal = value;
        ttk = k;
    }
    
    public boolean consec(int num) {
        tempList.add(num);
        tempMap.put(num, tempMap.getOrDefault(num, 0) + 1);
        if (tempList.size() < ttk) {
            return false;
        } else if (tempList.size() == ttk) {
            if (tempMap.getOrDefault(ttVal, 0) == ttk) {
                return true;
            }
        } else{
            int before = tempList.get(tempList.size() - ttk - 1);
            tempMap.put(before, tempMap.get(before) - 1);
            if (tempMap.getOrDefault(ttVal, 0) == ttk) {
                return true;
            }
        }
        return false;
    }
}

/**
 * Your DataStream object will be instantiated and called as such:
 * DataStream obj = new DataStream(value, k);
 * boolean param_1 = obj.consec(num);
 */
```



# 2529.正整数和负整数的最大计数

```java
class Solution {
    public int maximumCount(int[] nums) {
        int zzs = 0;
        int fzs = 0;
        for (int i = 0; i < nums.length; i++) {
            if (nums[i] > 0) {
                zzs++;
            } else if (nums[i] < 0) {
                fzs++;
            }
        }
        return Math.max(zzs, fzs);
    }
}
```

# 2530.执行K次操作后的最大分数

```java
class Solution {
    public long maxKelements(int[] nums, int k) {
        PriorityQueue<Integer> heap = new PriorityQueue<>((a, b) -> b - a);
        for (int i = 0; i < nums.length; i++) {
            heap.add(nums[i]);
        }
        long result = 0;
        while (k > 0) {
            int score = heap.poll();
            result += score;
            int temp = (score + 2) / 3;
            heap.add(temp);
            k--;
        }
        return result;
    }
}
```



# 2531.使字符串中不同字符的数目相等

```java
class Solution {
    public boolean isItPossible(String word1, String word2) {
        int[] word1List = new int[26];
        int[] word2List = new int[26];
        for (int i = 0; i < word1.length(); i++) {
            word1List[word1.charAt(i) - 'a']++;
        }
        for (int i = 0; i < word2.length(); i++) {
            word2List[word2.charAt(i) - 'a']++;
        }
        int have1zeroAnd2zero = 0;
        int have1oneAnd2zero = 0;
        int have1zeroAnd2one = 0;
        int have1oneAnd2one = 0;
        int have1someAnd2some = 0;
        int have1someAnd2zero = 0;
        int have1zeroAnd2some = 0;
        int have1someAnd2one = 0;
        int have1oneAnd2some = 0;
        int word1Cnt = 0;
        int word2Cnt = 0;
        for (int i = 0; i < word1List.length; i++) {
            if (word1List[i] > 0) {
                word1Cnt++;
            }
            if (word2List[i] > 0) {
                word2Cnt++;
            }
            if (word1List[i] == 0 && word2List[i] == 0) {
                have1zeroAnd2zero++;
            } else if (word1List[i] == 1 && word2List[i] == 0) {
                have1oneAnd2zero++;
            } else if (word1List[i] == 0 && word2List[i] == 1) {
                have1zeroAnd2one++;
            } else if (word1List[i] == 1 && word2List[i] == 1) {
                have1oneAnd2one++;
            } else if (word1List[i] > 1 && word2List[i] > 1) {
                have1someAnd2some++;
            } else if (word1List[i] > 1 && word2List[i] == 0) {
                have1someAnd2zero++;
            } else if (word1List[i] == 0 && word2List[i] > 1) {
                have1zeroAnd2some++;
            } else if (word1List[i] > 1 && word2List[i] == 1) {
                have1someAnd2one++;
            } else if (word1List[i] == 1 && word2List[i] > 1) {
                have1oneAnd2some++;
            }
        }
        if (word1Cnt == word2Cnt) {
            if (have1zeroAnd2one > 0 && have1oneAnd2zero > 0) {
                return true;
            } else if (have1oneAnd2one > 0 || have1oneAnd2some > 0 || have1someAnd2one > 0 || have1someAnd2some > 0) {
                return true;
            } else if (have1someAnd2zero > 0 && have1zeroAnd2some > 0) {
                return true;
            } else {
                return false;
            }
        } else if (word1Cnt - word2Cnt == 1) {
            if ((have1oneAnd2one > 0 || have1oneAnd2some > 0) && (have1oneAnd2some > 1 || have1someAnd2some > 0)) {
                return true;
            } else if (have1someAnd2zero > 0 && (have1oneAnd2some > 0 || have1someAnd2some > 0)) {
                return true;
            } else if (have1oneAnd2one > 0 && have1oneAnd2zero > 0) {
                return true;
            } else {
                return false;
            }
        } else if (word2Cnt - word1Cnt == 1) {
            if ((have1oneAnd2one > 0 || have1someAnd2one > 0) && (have1someAnd2one > 1 || have1someAnd2some > 0)) {
                return true;
            } else if (have1zeroAnd2some > 0 && (have1someAnd2one > 0 || have1someAnd2some > 0)) {
                return true;
            } else if (have1oneAnd2one > 0 && have1zeroAnd2one > 0) {
                return true;
            } else {
                return false;
            }
        } else if (word1Cnt - word2Cnt == 2) {
            if (have1oneAnd2one > 1) {
                return true;
            } else if (have1oneAnd2zero > 1 && (have1someAnd2some > 0 || have1oneAnd2some > 0)) {
                return true;
            } else {
                return false;
            }
        } else if (word2Cnt - word1Cnt == 2) {
            if (have1oneAnd2one > 1) {
                return true;
            } else if (have1zeroAnd2one > 1 && (have1someAnd2some > 0 || have1someAnd2one > 0)) {
                return true;
            } else {
                return false;
            }
        }
        return false;
    }
}
```



# 2535.数组元素和与数字和的绝对差

```java
class Solution {
    public int differenceOfSum(int[] nums) {
        int sum = 0;
        int tempSum = 0;
        for (int i = 0; i < nums.length; i++) {
            sum += nums[i];
            int temp = nums[i];
            while (temp > 0) {
                tempSum = tempSum + temp % 10;
                temp /= 10;
            }
        }
        return Math.abs(sum - tempSum);
    }
}
```

# 2540.最小公共值

```java
class Solution {
    public int getCommon(int[] nums1, int[] nums2) {
        Set<Integer> tempSet = new HashSet<>();
        for (int i = 0; i < nums1.length; i++) {
            tempSet.add(nums1[i]);
        }
        for (int i = 0; i < nums2.length; i++) {
            if (tempSet.contains(nums2[i])) {
                return nums2[i];
            }
        }
        return -1;
    }
}
```

# 2541.使数组中所有元素相等的最小操作数Ⅱ

```java
class Solution {
    public long minOperations(int[] nums1, int[] nums2, int k) {
        long addK = 0;
        long subtractK = 0;
        if (k == 0) {
            String str1 = Arrays.toString(nums1);
            String str2 = Arrays.toString(nums2);
            if (str1.equals(str2)) {
                return 0;
            } else {
                return -1;
            }
        }
        for (int i = 0; i < nums1.length; i++) {
            int need = nums1[i] - nums2[i];
            if (need % k != 0) {
                return -1;
            }
            if (need > 0) {
                addK += need / k;
            } else {
                subtractK += -need / k;
            }
        }
        if (addK != subtractK) {
            return -1;
        }
        return addK;
    }
}
```



# 2544.交替数字和

```java
class Solution {
    public int alternateDigitSum(int n) {
        int result = 0;
        int index = 0;
        while (n > 0) {
            result += Math.pow(-1, index) * (n % 10);
            n /= 10;
            index++;
        }
        return (int) Math.pow(-1, index - 1) * result;
    }
}
```

# 2545.根据第K场考试的分数排序

```java
class Solution {
    public int[][] sortTheStudents(int[][] score, int k) {
        Arrays.sort(score, (a, b) -> {
            return b[k] - a[k];
        });
        return score;
    }
}
```



# 2549.统计桌面上的不同数字

```java
class Solution {
    public int distinctIntegers(int n) {
        return n == 1 ? 1 : n - 1;
    }
}
```



# 2552.统计上升四元组

```java
/**
 * 超时
 */
class Solution {
    int maxn = 4001;
    int[][] less = new int[maxn][maxn];
    int[][] bigger = new int[maxn][maxn];
    public long countQuadruplets(int[] nums) {
        int n = nums.length;
        for (int i = 1; i <= n; i++) {
            less[0][i] = 0;
        }
        for (int i = 1; i < n; i++) {
            int x = nums[i - 1];
            for (int j = 1; j <= n; j++) {
                if (x < j) {
                    less[i][j] = less[i - 1][j] + 1;
                } else {
                    less[i][j] = less[i - 1][j];
                }
            }
        }
        for (int i = 1; i <= n; i++) {
            bigger[n - 1][i] = 0;
        }
        for (int i = n - 2; i >= 0 ;i--) {
            int x = nums[i + 1];
            for (int j = 1; j <= n; j++) {
                if (x > j) {
                    bigger[i][j] = bigger[i + 1][j] + 1;
                } else {
                    bigger[i][j] = bigger[i + 1][j];
                }
            }
        }
        long result = 0;
        for (int j = 0; j < n; j++) {
            for (int k = j + 1; k < n; k++) {
                if (nums[k] < nums[j]) {
                    result += less[j][nums[k]] * bigger[k][nums[j]];
                }
            }
        }
        return result;
    }
}
```

```java
class Solution {
    public long countQuadruplets(int[] nums) {
        int n = nums.length;
        int[][] great = new int[n][n + 1];
        for (int k = n - 2; k > 0; k--) {
            great[k] = great[k + 1].clone();
            for (int x = nums[k + 1] - 1; x > 0; x--)
                great[k][x]++;
        }

        long ans = 0;
        for (int j = 1; j < n - 2; j++)
            for (int k = j + 1; k < n - 1; k++) {
                int x = nums[k];
                if (nums[j] > x)
                    ans += (x - n + 1 + j + great[j][x]) * great[k][nums[j]];
            }
        return ans;
    }
}
```

# 2553.分割数组中数字的数位

```java
class Solution {
    public int[] separateDigits(int[] nums) {
        List<Integer> tempList = new ArrayList<>();
        for (int i = 0; i < nums.length; i++) {
            List<Integer> temp = new ArrayList<>();
            int tempInt = nums[i];
            while (tempInt > 0) {
                temp.add(tempInt % 10);
                tempInt /= 10;
            }
            Collections.reverse(temp);
            tempList.addAll(temp);
        }
        int[] result = new int[tempList.size()];
        for (int i = 0; i < tempList.size(); i++) {
            result[i] = tempList.get(i);
        }
        return result;
    }
}
```

# 2558.从数量最多的堆取走礼物

```java
class Solution {
    public long pickGifts(int[] gifts, int k) {
        while (k > 0) {
            Arrays.sort(gifts);
            gifts[gifts.length - 1] = (int) Math.sqrt(gifts[gifts.length - 1]);
            k--;
        }
        long sum = 0;
        for (int i = 0; i < gifts.length; i++) {
            sum += (long) gifts[i];
        }
        return sum;
    }
}
```

# 2559.统计范围内的元音字符串数

```java
class Solution {
    public int[] vowelStrings(String[] words, int[][] queries) {
        int[] s = new int[words.length];
        Set<Character> yuanSet = new HashSet<>();
        yuanSet.add('a');
        yuanSet.add('e');
        yuanSet.add('i');
        yuanSet.add('o');
        yuanSet.add('u');
        if (yuanSet.contains(words[0].charAt(0)) && yuanSet.contains(words[0].charAt(words[0].length() - 1))) {
            s[0] = 1;
        }
        for (int i = 1; i < words.length; i++) {
            s[i] = s[i - 1];
            if (yuanSet.contains(words[i].charAt(0)) && yuanSet.contains(words[i].charAt(words[i].length() - 1))) {
                s[i]++;
            }
        }
        int[] result = new int[queries.length];
        for (int i = 0; i < queries.length; i++) {
            int left = queries[i][0];
            int right = queries[i][1];
            if (left == 0) {
                result[i] = s[right];
            } else {
                result[i] = s[right] - s[left - 1];
            }
        }
        return result;
    }
}
```



# 2562.找出数组的串联值

```java
class Solution {
    public long findTheArrayConcVal(int[] nums) {
        int left = 0;
        int right = nums.length - 1;
        long result = 0;
        while (left < right) {
            long temp = (long) Integer.valueOf(String.valueOf(nums[left++]) + String.valueOf(nums[right--]));
            result += temp;
        }
        if (left == right) {
            result += nums[left];
        }
        return result;
    }
}
```



# 2563.统计公平数对的数目

```java
class Solution {
    public long countFairPairs(int[] nums, int lower, int upper) {
        Arrays.sort(nums);
        long lessLower = lessThan(nums, lower - 1);
        long lessUpper = lessThan(nums, upper);
        return lessUpper - lessLower;
    }
    public long lessThan(int[] nums, int value) {
        int left = 0; 
        int right = nums.length - 1;
        long result = 0;
        while (left < right) {
            if(nums[left] + nums[right] <= value) {
                result += (right - left);
                left++;
            } else {
                right--;
            }
        }
        return result;
    }

}
```

# 2566.替换一个数字后的最大差值

```java
class Solution {
    public int minMaxDifference(int num) {
        String numString = String.valueOf(num);
        char[] numStringChar = numString.toCharArray();
        char maxReNumber = ' ';
        char minReNumber = numStringChar[0];
        int max = 0;
        int min = 0;
        for (int i = 0; i < numStringChar.length; i++) {
            if (numStringChar[i] != '9' && maxReNumber == ' ') {
                maxReNumber = numStringChar[i];
            }
            if (numStringChar[i] == maxReNumber) {
                max = max * 10 + 9;
            } else {
                max = max * 10 + (numStringChar[i] - '0');
            }
            if (numStringChar[i] == minReNumber) {
                min = min * 10 + 0;
            } else {
                min = min * 10 + (numStringChar[i] - '0');
            }
        }
        return max - min;
    }
}
```

# 2567.修改两个元素的最小分数

```java
class Solution {
    public int minimizeSum(int[] nums) {
        Arrays.sort(nums);
        int left = nums[nums.length- 3] - nums[0];
        int mid = nums[nums.length - 2] - nums[1];
        int right = nums[nums.length - 1] - nums[2];
        return Math.min(left, Math.min(right, mid));
    }
}
```



# 2570.合并两个二维数组 - 求和法

```java
class Solution {
    public int[][] mergeArrays(int[][] nums1, int[][] nums2) {
        int[] temp = new int[1001];
        Set<Integer> idSet = new HashSet<>();
        for (int i = 0; i < nums1.length; i++) {
            temp[nums1[i][0]] += nums1[i][1];
            idSet.add(nums1[i][0]);
        }
        for (int i = 0; i < nums2.length; i++) {
            temp[nums2[i][0]] += nums2[i][1];
            idSet.add(nums2[i][0]);
        }
        int[][] result = new int[idSet.size()][2];
        int index = 0;
        for (int i = 0; i < temp.length; i++) {
            if (temp[i] != 0) {
                result[index][0] = i;
                result[index][1] = temp[i];
                index++;
            }
        }
        return result;
    }
}
```



# 2574.左右元素和的差值

```java
class Solution {
    public int[] leftRightDifference(int[] nums) {
        int[] leftSum = new int[nums.length];
        int[] rightSum = new int[nums.length];
        for (int i = 1; i < nums.length; i++) {
            leftSum[i] = nums[i - 1] + leftSum[i - 1];
        }
        for (int i = nums.length - 2; i >= 0; i--) {
            rightSum[i] = nums[i + 1] + rightSum[i + 1];
        }
        int[] answer = new int[nums.length];
        for (int i = 0; i < nums.length; i++) {
            answer[i] = Math.abs(leftSum[i] - rightSum[i]);
        }
        return answer;
    }
}
```



# 2578.最小和分割

```java
class Solution {
    public int splitNum(int num) {
        char[] numChar = String.valueOf(num).toCharArray();
        Arrays.sort(numChar);
        int num1 = 0;
        int num2 = 0;
        for (int i = 0; i < numChar.length; i++) {
            int temp1 = numChar[i] - '0';
            num1 = num1 * 10 + temp1;
            if (i + 1 < numChar.length) {
                int temp2 = numChar[i + 1] - '0';
                num2 = num2 * 10 + temp2;
                i++;
            }
        }
        return num1 + num2;
    }
}
```

# 2580.统计将重叠区间合并成组的方案数

```java
class Solution {
    public int countWays(int[][] ranges) {
        Arrays.sort(ranges, new Comparator<int[]>() {
            @Override
            public int compare(int[] o1, int[] o2) {
                return Integer.compare(o1[0], o2[0]);
            }
        });
        int max = ranges[0][1];
        int mod = (int) 1e9 + 7;
        int result = 2;
        for (int i = 1; i < ranges.length; i++) {
            if (ranges[i][0] > max) {
                result = (result << 1) % mod;
            }
            max = Math.max(max, ranges[i][1]);
        }
        return result;
    }
}
```



# 2582.递枕头

```java
class Solution {
    public int passThePillow(int n, int time) {
        time = time % ((n - 1) * 2);
        int result = 1;
        for (int i = 1; i <= time; i++) {
            if (i < n) {
                result++;
            } else {
                result--;
            }
        }
        return result;
    }
}
```



# 2586.统计范围内的元音字符串数

```java
class Solution {
    public int vowelStrings(String[] words, int left, int right) {
        Set<Character> yuanSet = new HashSet<>();
        yuanSet.add('a');
        yuanSet.add('e');
        yuanSet.add('i');
        yuanSet.add('o');
        yuanSet.add('u');
        int result = 0;
        for (int i = left; i <= right; i++) {
            if (yuanSet.contains(words[i].charAt(0)) && yuanSet.contains(words[i].charAt(words[i].length() - 1))) {
                result++;
            }
        }
        return result;
    }
}
```



# 2591.将钱分给最多的儿童

```java
class Solution {
    public int distMoney(int money, int children) {
        if (money < children) {
            return -1;
        }
        int moreMoney = money - children;
        int cnt = Math.min(moreMoney / 7, children);
        int lastMoney = moreMoney - cnt * 7;
        if ((lastMoney > 0 && cnt == children) || (lastMoney == 3 && (children - cnt) == 1)) {
            cnt--;
        }
        return cnt;
    }
}
```

# 2595.奇偶位数

```java
class Solution {
    public int[] evenOddBit(int n) {
        int even = 0;
        int odd = 0;
        for (int i = 31; i >= 0; i--) {
            if (((n >> i) & 1) == 1) {
                if (i % 2 ==0) {
                    even++;
                } else {
                    odd++;
                }
            }
        }
        return new int[]{even, odd};
    }
}
```

# 2598.执行操作后的最大MEX

```java
class Solution {
    public int findSmallestInteger(int[] nums, int value) {
        Map<Integer, Integer> tempMap = new HashMap<>();
        for (int i = 0; i < nums.length; i++) {
            if (nums[i] < 0) {
                nums[i] += (-nums[i] / value + 1) * value;
            }
            int need = nums[i] % value;
            tempMap.put(need, tempMap.getOrDefault(need, 0) + 1);
        }
        int minCnt = tempMap.getOrDefault(0, 0);
        int minCntNum = 0;
        if (minCnt == 0) {
            return 0;
        }
        for (int i = 1; i < value; i++) {
            int val = tempMap.getOrDefault(i, 0);
            if (val == 0) {
                return i;
            }
            if (val < minCnt) {
                minCnt = val;
                minCntNum = i;
            }
        }
        return minCnt * value + minCntNum;
    }
}
```



# 2600.K件物品的最大和

```java
class Solution {
    public int kItemsWithMaximumSum(int numOnes, int numZeros, int numNegOnes, int k) {
        int result = 0;
        result += Math.min(numOnes, k);
        k -= numOnes;
        if (k > 0) {
            k -= numZeros;
        }
        if (k > 0) {
            result -= k;
        }
        return result;
    }
}
```

