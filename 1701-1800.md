# 1702.修改后的最大二进制字符串

```java
class Solution {
    public String maximumBinaryString(String binary) {
        char[] tempChar = new char[binary.length()];
        Arrays.fill(tempChar, '1');
        int index = -1;
        int cnt = 0;
        for (int i = 0; i < binary.length(); i++) {
            char temp = binary.charAt(i);
            if (temp == '0') {
                if (index == -1) {
                    index = i;
                }
                cnt++;
            }
        }
        if (cnt >= 1) {
            tempChar[index + cnt - 1] = '0';
        }
        return new String(tempChar);
    }
}
```



# 1704.判断字符串的两半是否相似

```java
class Solution {
    public boolean halvesAreAlike(String s) {
        char[] c = s.toCharArray();
        String h = "aeiouAEIOU";
        int len = c.length;
        int temp = 0;
        for (int i = 0; i < len / 2; i++) {
            if (h.indexOf(c[i]) >= 0) {
                temp++;
            }
        }
        for (int i = len / 2; i < len; i++) {
            if (h.indexOf(c[i]) >= 0) {
                temp--;
            }
        }
        return temp == 0;
    }
}
```

# 1710.卡车上的最大单元数

```java
class Solution {
    public int maximumUnits(int[][] boxTypes, int truckSize) {
        Arrays.sort(boxTypes, new Comparator<int[]>() {
            public int compare(int[] o1, int[] o2) {
                return o2[1] - o1[1];
            }
        });
        int result = 0;
        for (int i = 0; i < boxTypes.length; i++) {
            int size = boxTypes[i][1];
            int count = boxTypes[i][0];
            if (count < truckSize) {
                result += count * size;
                truckSize -= count;
                continue;
            } else {
                result += truckSize * size;
                return result;
            }
        }
        return result;
    }
}
```

# 1711.大餐计数

```java
class Solution {
    public int countPairs(int[] deliciousness) {
        int mod = (int) 1e9 + 7;
        Map<Integer, Integer> tempMap = new HashMap<>();
        for (int i = 0; i < deliciousness.length; i++) {
            tempMap.put(deliciousness[i], tempMap.getOrDefault(deliciousness[i], 0) + 1);
        }
        long result = 0;
        long equalRes = 0;
        for (Integer key : tempMap.keySet()) {
            for (int i = 0; i <= 21; i++) {
                int tempRes = (int) Math.round(Math.pow(2, i));
                int otherKey = tempRes - key;
                int otherCnt = tempMap.getOrDefault(otherKey, 0);
                int cnt = tempMap.get(key);
                if (key == otherKey) {
                    equalRes += (long) otherCnt * (otherCnt - 1) / 2;
                    equalRes = equalRes % mod;
                } else if (otherCnt != 0) {
                    result += (long) cnt * otherCnt;
                    result = result % mod;
                }
            }
        }
        return (int) ((result / 2 + equalRes) % mod);
    }
}
```



# 1716.计算力扣银行的钱

```java
class Solution {
    public int totalMoney(int n) {
        int sum = 0;
        int weak = 1;
        while (n > 0) {
            int day = Math.min(n, 7);
            int tempSum = (weak + weak + day - 1) * day / 2;
            sum += tempSum;
            weak++;
            n -= day;
        }
        return sum;
    }
}
```



# 1720.解码异或后的数组

```java
class Solution {
    public int[] decode(int[] encoded, int first) {
        int[] arr = new int[encoded.length + 1];
        arr[0] = first;
        for (int i = 1; i < arr.length; i++) {
            arr[i] = arr[i - 1] ^ encoded[i - 1];
        }
        return arr;
    }
}
```



# 1721.交换链表中的节点

```java
/**
 * Definition for singly-linked list.
 * public class ListNode {
 *     int val;
 *     ListNode next;
 *     ListNode() {}
 *     ListNode(int val) { this.val = val; }
 *     ListNode(int val, ListNode next) { this.val = val; this.next = next; }
 * }
 */
class Solution {
    public ListNode swapNodes(ListNode head, int k) {
        ListNode newHead = new ListNode(0, head);
        ListNode left = newHead;
        ListNode right = newHead;
        while (k > 0) {
            right = right.next;
            k--;
        }
        ListNode firstNode = right;
        while (right != null) {
            left = left.next;
            right = right.next;
        }
        ListNode lastNode = left;
        int temp = firstNode.val;
        firstNode.val = lastNode.val;
        lastNode.val = temp;
        return newHead.next;
    }
}
```

# 1725.可以形成最大正方形的矩形数目

```java
class Solution {
    public int countGoodRectangles(int[][] rectangles) {
        int max = 0;
        int cnt = 0;
        for (int i = 0; i < rectangles.length; i++) {
            int temp = Math.min(rectangles[i][0], rectangles[i][1]);
            if (temp > max) {
                max = temp;
                cnt = 1;
            } else if (temp == max) {
                cnt++;
            }
        }
        return cnt;
    }
}
```

# 1726.同积元组

```java
class Solution {
    public int tupleSameProduct(int[] nums) {
        Map<Integer, Integer> tempMap = new HashMap<>();
        for (int i = 0; i < nums.length; i++) {
            for (int j = i + 1; j < nums.length; j++) {
                tempMap.put(nums[i] * nums[j], tempMap.getOrDefault(nums[i] * nums[j], 0) + 1);
            }
        }
        int result = 0;
        for (Integer key : tempMap.keySet()) {
            int cnt = tempMap.get(key);
            if (cnt > 1) {
                int tempRes = (cnt) * (cnt - 1) / 2;
                result += tempRes;
            }
        }
        return result * 8;
    }
}
```



# 1729.求关注者的数量

```sql
# Write your MySQL query statement below
select user_id, count(*) Followers_count from Followers group by user_id order by user_id;
```

# 1731.每位经理的下属员工数量

```sql
# Write your MySQL query statement below
select bb.employee_id, bb.name, aa.reports_count, aa.average_age from (
select reports_to, count(*) reports_count, round(avg(age), 0) average_age from Employees where reports_to is not null group by reports_to) aa, Employees bb where aa.reports_to = bb.employee_id order by bb.employee_id;
```



# 1732.找到最高海拔

```java
class Solution {
    public int largestAltitude(int[] gain) {
        int[] motain = new int[gain.length + 1];
        int max = 0;
        for (int i = 1; i < motain.length; i++) {
            motain[i] = gain[i - 1] + motain[i - 1];
            max = Math.max(max, motain[i]);
        }
        return max;
    }
}
```

# 1733.需要教语言的最少人数

```java
class Solution {
    public int minimumTeachings(int n, int[][] languages, int[][] friendships) {
        boolean[][] isUse = new boolean[languages.length][n];
        for (int i = 0; i < languages.length; i++) {
            for (int j = 0;j < languages[i].length; j++) {
                isUse[i][languages[i][j] - 1] = true;
            }
        }
        Set<Integer> unUseSet = new HashSet<>();
        next:
        for (int i = 0; i < friendships.length; i++) {
            int user1 = friendships[i][0] - 1;
            int user2 = friendships[i][1] - 1;
            for (int j = 0; j < languages[user1].length; j++) {
                if (isUse[user2][languages[user1][j] - 1]) {
                    continue next;
                }
            }
            unUseSet.add(user1);
            unUseSet.add(user2);
        }
        int[] useLang = new int[n];
        for (Integer key : unUseSet) {
            for (int i = 0; i < isUse[key].length; i++) {
                if (isUse[key][i]) {
                    useLang[i]++;
                }
            }
        }
        Arrays.sort(useLang);
        return unUseSet.size() - useLang[n - 1];
    }
}
```



# 1736.替换隐藏数字得到的最晚时间

```java
class Solution {
    public String maximumTime(String time) {
        String[] timeSplit = time.split(":");
        String hour = timeSplit[0];
        String minute = timeSplit[1];
        if (hour.charAt(0) == '?' && hour.charAt(1) == '?') {
            hour = "23";
        }if (hour.charAt(0) == '?') {
            if (hour.charAt(1) <= '3') {
                hour = "2" + hour.charAt(1);
            } else {
                hour = "1" + hour.charAt(1);
            }
        }if (hour.charAt(1) == '?') {
            if (hour.charAt(0) <= '1') {
                hour = hour.charAt(0) + "9";
            } else {
                hour = hour.charAt(0) + "3";
            }
        }
        if (minute.charAt(0) == '?' && minute.charAt(1) == '?') {
            minute = "59";
        }if (minute.charAt(0) == '?') {
            minute = "5" + minute.charAt(1);
        }if (minute.charAt(1) == '?') {
            minute = minute.charAt(0) + "9";
        }
        return hour + ":" + minute;
    }
}
```

# 1737.满足三条件之一需改变的最少字符数

```java
class Solution {
    public int minCharacters(String a, String b) {
        int[] aa = new int[26];
        int[] bb = new int[26];
        for (int i = 0; i < a.length(); i++) {
            aa[a.charAt(i) - 'a']++;
        }
        for (int i = 0; i < b.length(); i++) {
            bb[b.charAt(i) - 'a']++;
        }
        int lenA = a.length();
        int lenB = b.length();
        int aMoreB = Integer.MAX_VALUE;
        int bMoreA = Integer.MAX_VALUE;
        int aEqualB = Integer.MAX_VALUE;
        for (int i = 0; i < aa.length; i++) {
            aEqualB = Math.min(aEqualB, lenA + lenB - aa[i] - bb[i]);
        }
        int leftA = aa[0];
        int leftB = bb[0];
        int rightA = lenA - leftA;
        int rightB = lenB - leftB;
        for (int i = 1; i < aa.length; i++) {
            aMoreB = Math.min(aMoreB, leftA + rightB);
            bMoreA = Math.min(bMoreA, leftB + rightA);
            leftA += aa[i];
            leftB += bb[i];
            rightA -= aa[i];
            rightB -= bb[i];
        }
        return Math.min(aEqualB, Math.min(aMoreB, bMoreA));
    }
}
```



# 1741.查找每个员工花费的总时间

```sql
# Write your MySQL query statement below
select event_day day, emp_id, sum(out_time - in_time) total_time from Employees group by event_day, emp_id;
```



# 1742.盒子中小球的最大数量

```java
class Solution {
    public int countBalls(int lowLimit, int highLimit) {
        Map<Integer, Integer> resultMap = new HashMap<>();
        int maxNumber = 0;
        for (int i = lowLimit; i <= highLimit; i++) {
            int number = 0;
            int tempInt = i;
            while (tempInt > 0) {
                number += tempInt % 10;
                tempInt /= 10;
            }
            int cnt = resultMap.getOrDefault(number, 0) + 1;
            resultMap.put(number, cnt);
            maxNumber = Math.max(maxNumber, cnt);
        }
        return maxNumber;
    }
}
```

# 1748.唯一元素的和

```java
class Solution {
    public int sumOfUnique(int[] nums) {
        Set<Integer> onlySet = new HashSet<>();
        Set<Integer> notOnlySet = new HashSet<>();
        for (int i = 0; i < nums.length; i++) {
            if (notOnlySet.contains(nums[i])) {
                continue;
            }
            if (onlySet.contains(nums[i])) {
                onlySet.remove(nums[i]);
                notOnlySet.add(nums[i]);
                continue;
            }
            onlySet.add(nums[i]);
        }
        int result = 0;
        for (Integer key : onlySet) {
            result += key;
        }
        return result;
    }
}
```

# 1749.任意子数组和的绝对值的最大值

```java
class Solution {
    public int maxAbsoluteSum(int[] nums) {
        int[] s = new int[nums.length];
        int max = Math.max(nums[0], 0);
        int min = Math.min(nums[0], 0);
        s[0] = nums[0];
        for (int i = 1; i < nums.length; i++) {
            s[i] = s[i - 1] + nums[i];
            max = Math.max(max, s[i]);
            min = Math.min(min, s[i]);
        }
        return max - min;
    }
}
```



# 1752.检查数组是否经排序和轮转得到

```java
class Solution {
    public boolean check(int[] nums) {
        StringBuilder sb = new StringBuilder();
        StringBuilder sbA = new StringBuilder();
        int[] numsA = Arrays.copyOf(nums, nums.length);
        Arrays.sort(numsA);
        for (int i = 0; i < nums.length; i++) {
            sb.append(nums[i]).append(",");
            sbA.append(numsA[i]).append(",");
        }
        String s = sb.toString() + sb.toString();
        String target = sbA.toString();
        if (s.indexOf(target) != -1) {
            return true;
        }
        return false;
    }
}
```



# 1758.生成交替二进制字符串的最少操作数

```java
class Solution {
    public int minOperations(String s) {
        int len = s.length();
        int cnt = 0;
        for (int i = 0; i < len; i++) {
            if (i % 2 == 0 && s.charAt(i) != '0') {
                cnt++;
            }
            if (i % 2 != 0 && s.charAt(i) != '1') {
                cnt++;
            }
        }
        return Math.min(cnt, len - cnt);
    }
}
```



# 1763.最长的美好子字符串

```java
class Solution {
    public String longestNiceSubstring(String s) {
        int result = 0;
        String resultString = "";
        for (int i = 0; i < s.length(); i++) {
            int lower = 0;
            int upper = 0;
            for (int j = i; j < s.length(); j++) {
                char temp = s.charAt(j);
                if (Character.isLowerCase(temp)) {
                    lower |= 1 << (temp - 'a');
                } else {
                    upper |= 1 << (temp - 'A');
                }
                if (lower == upper && j - i + 1 > result) {
                    result = j - i + 1;
                    resultString = s.substring(i, j + 1);
                }
            }
        }
        return resultString;
    }
}
```



# 1768.交替合并字符串

```java
class Solution {
    public String mergeAlternately(String word1, String word2) {
        StringBuilder sb = new StringBuilder();
        int word1Index = 0;
        int word2Index = 0;
        while (word1Index < word1.length() && word2Index < word2.length()) {
            sb.append(word1.charAt(word1Index++)).append(word2.charAt(word2Index++));
        }
        while (word1Index < word1.length()) {
            sb.append(word1.charAt(word1Index++));
        }
        while (word2Index < word2.length()) {
            sb.append(word2.charAt(word2Index++));
        }
        return sb.toString();
    }
}
```

# 1773.统计匹配检索规则的物品数量

```java
class Solution {
    public int countMatches(List<List<String>> items, String ruleKey, String ruleValue) {
        int index = -1;
        if ("type".equals(ruleKey)) {
            index = 0;
        } else if ("color".equals(ruleKey)) {
            index = 1;
        } else {
            index = 2;
        }
        int result = 0;
        for (int i = 0; i < items.size(); i++) {
            if (ruleValue.equals(items.get(i).get(index))) {
                result++;
            }
        }
        return result;
    }
}
```

# 1775.通过最少操作次数使数组的和相等

```java
class Solution {
    public int minOperations(int[] nums1, int[] nums2) {
        int sum1 = 0;
        int sum2 = 0;
        int[] number1 = new int[7];
        int[] number2 = new int[7];
        for (int i = 0; i < nums1.length; i++) {
            number1[nums1[i]]++;
            sum1 += nums1[i];
        }
        for (int i = 0; i < nums2.length; i++) {
            number2[nums2[i]]++;
            sum2 += nums2[i];
        }
        if (sum1 == sum2) {
            return 0;
        }
        if (sum2 > sum1) {
            int temp = sum1;
            sum1 = sum2;
            sum2 = temp;
            int[] tempArray = number1;
            number1 = number2;
            number2 = tempArray;
        }
        int result = 0;
        int needNum = sum1 - sum2;
        for (int i = number1.length - 1; i > 1; i--) {
            int temp1 = number1[i];
            int temp2 = number2[7 - i];
            int canDo = i - 1;
            int needDo = (needNum + canDo - 1) / canDo;
            if (needDo > temp1 + temp2) {
                result += temp1 + temp2;
                needNum -= canDo * (temp1 + temp2);
            } else {
                result += needDo;
                needNum = 0;
            }
        }
        return needNum == 0 ? result : -1;
    }
}
```



# 1781.所有子字符串美丽值之和

```java
class Solution {
    public int beautySum(String s) {
        int result = 0;
        for (int i = 0; i < s.length(); i++) {
            int[] tempInt = new int[26];
            for (int j = i; j < s.length(); j++) {
                tempInt[s.charAt(j) - 'a']++;
                int tempMax = 0;
                int tempMin = 999;
                for (int k = 0; k < tempInt.length; k++) {
                    if (tempInt[k] != 0) {
                        tempMax = Math.max(tempMax, tempInt[k]);
                        tempMin = Math.min(tempMin, tempInt[k]);
                    }
                }
                result += tempMax - tempMin;
            }
        }
        return result;
    }
}
```



# 1784.检查二进制字符串字段

```java
class Solution {
    public boolean checkOnesSegment(String s) {
        int cnt = 0;
        for (int i = 0; i < s.length(); i++) {
            char temp = s.charAt(i);
            if (temp == '1') {
                cnt++;
                if (cnt > 1) {
                    return false;
                }
                while (i < s.length() && s.charAt(i) == '1') {
                    i++;
                }
                i--;
            }
        }
        return true;
    }
}
```

# 1785.构成特定和需要添加的最少元素

```java
class Solution {
    public int minElements(int[] nums, int limit, int goal) {
        long sum = 0;
        for (int i = 0; i < nums.length; i++) {
            sum += nums[i];
        }
        long need = Math.abs(sum - goal);
        return (int) ((need + limit - 1) / limit);
    }
}
```



# 1789.员工的直属部门

```sql
# Write your MySQL query statement below
select employee_id, department_id from Employee where primary_flag = 'Y'
union all
select employee_id, department_id from Employee where employee_id in 
(select a.employee_id from 
(select employee_id, count(*) cnt from Employee group by employee_id) a
where cnt = 1
);
```



# 1790.仅执行一次字符串交换能否使两个字符串相等

```java
class Solution {
    public boolean areAlmostEqual(String s1, String s2) {
        List<Character> s1List = new ArrayList<>();
        List<Character> s2List = new ArrayList<>();
        for (int i = 0; i < s1.length(); i++) {
            char tempChar1 = s1.charAt(i);
            char tempChar2 = s2.charAt(i);
            if (tempChar1 != tempChar2) {
                s1List.add(tempChar1);
                s2List.add(tempChar2);
            }
            if (s1List.size() > 2) {
                return false;
            }
        }
        if (s1List.size() == 0) {
            return true;
        }
        if (s1List.size() != 2) {
            return false;
        }
        if (s1List.get(0) == s2List.get(1) && s1List.get(1) == s2List.get(0)) {
            return true;
        }
        return false;
    }
}
```

# 1791.找出星型图的中心节点

```java
class Solution {
    public int findCenter(int[][] edges) {
        if (edges[0][0] == edges[1][0] || edges[0][0] == edges[1][1]) {
            return edges[0][0];
        }
        return edges[0][1];
    }
}
```



# 1796.字符串中第二大的数字

```java
class Solution {
    public int secondHighest(String s) {
        int max = -1;
        int secondMax = -1;
        for (int i = 0; i < s.length(); i++) {
            if (Character.isLetter(s.charAt(i))) {
                continue;
            }
            int number = s.charAt(i) - '0';
            if (number > max) {
                secondMax = max;
                max = number;
            } else if (number < max && number > secondMax) {
                secondMax = number;
            }
        }
        return secondMax;
    }
}
```

# 1798.你能构造出连续值的最大数目

```java
class Solution {
    public int getMaximumConsecutive(int[] coins) {
        Arrays.sort(coins);
        int result = 0;
        for (int coin : coins) {
            if (coin > result + 1) {
                break;
            }
            result += coin;
        }
        return result + 1;
    }
}
```



# 1800.最大升序子数组和

```java
class Solution {
    public int maxAscendingSum(int[] nums) {
        int result = nums[0];
        for (int i = 0; i < nums.length - 1; i++) {
            if (nums[i + 1] > nums[i]) {
                int temp = nums[i];
                while (i < nums.length - 1 && nums[i + 1] > nums[i]) {
                    temp += nums[i + 1];
                    i++;
                }
                result = Math.max(result, temp);
            } else {
                result = Math.max(result, nums[i]);
            }
        }
        return result;
    }
}
```

