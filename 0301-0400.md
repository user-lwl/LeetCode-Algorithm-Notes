# 303.区域和检索 - 数组不可变

```java
class NumArray {
    int[] number;

    public NumArray(int[] nums) {
        int len = nums.length;
        number = new int[len];
        for (int i = 0; i < len; i++) {
            number[i] = nums[i];
        }
    }
    
    public int sumRange(int left, int right) {
        int sum = 0;
        for (int i = left; i <= right; i++) {
            sum += number[i];
        }
        return sum;
    }
}

/**
 * Your NumArray object will be instantiated and called as such:
 * NumArray obj = new NumArray(nums);
 * int param_1 = obj.sumRange(left,right);
 */
```

```java
class NumArray {
    int[] sum;

    public NumArray(int[] nums) {
        int len = nums.length;
        sum = new int[len + 1];
        for (int i = 0; i < len; i++) {
            sum[i + 1] = nums[i] + sum[i];
        }
    }
    
    public int sumRange(int left, int right) {
        return sum[right + 1] - sum[left];
    }
}

/**
 * Your NumArray object will be instantiated and called as such:
 * NumArray obj = new NumArray(nums);
 * int param_1 = obj.sumRange(left,right);
 */
```

# 304.二维区域和检索 - 矩阵不可变

```java
class NumMatrix {

    int[][] prefix;

    public NumMatrix(int[][] matrix) {
        prefix = new int[matrix.length][matrix[0].length + 1];
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 1; j < prefix[i].length; j++) {
                prefix[i][j] = prefix[i][j - 1] + matrix[i][j - 1];
            }
        }
    }
    
    public int sumRegion(int row1, int col1, int row2, int col2) {
        int result = 0;
        for (int i = row1; i <= row2; i++) {
            result += prefix[i][col2 + 1] - prefix[i][col1];
        }
        return result;
    }
}

/**
 * Your NumMatrix object will be instantiated and called as such:
 * NumMatrix obj = new NumMatrix(matrix);
 * int param_1 = obj.sumRegion(row1,col1,row2,col2);
 */
```



# 309.买卖股票的最佳时机含冷冻期

```java
class Solution {
    public int maxProfit(int[] prices) {
        int[][] dp = new int[prices.length][3];
        // 当天买入或之前买入今天不操作
        dp[0][0] = -prices[0];
        // 当天卖出
        dp[0][1] = 0;
        // 昨天或之前卖出，今天不操作
        dp[0][2] = 0;
        for (int i = 1; i < prices.length; i++) {
            dp[i][0] = Math.max(dp[i - 1][0], dp[i - 1][2] - prices[i]);
            dp[i][1] = dp[i - 1][0] + prices[i];
            dp[i][2] = Math.max(dp[i - 1][1], dp[i - 1][2]);
        }
        return Math.max(dp[dp.length - 1][1], dp[dp.length - 1][2]);
    }
}
```



# 310.最小高度树

```java
class Solution {
    public List<Integer> findMinHeightTrees(int n, int[][] edges) {
        List<Integer> result = new ArrayList<>();
        if (n == 1) {
            result.add(0);
            return result;
        }
        List<Integer>[] adj= new List[n];
        for (int i = 0; i < n; i++) {
            adj[i] = new ArrayList<>();
        }
        for (int[] edge : edges) {
            adj[edge[0]].add(edge[1]);
            adj[edge[1]].add(edge[0]);
        }
        int[] parent = new int[n];
        Arrays.fill(parent, -1);
        int x = findLongNode(0, parent, adj);
        int y = findLongNode(x, parent, adj);
        List<Integer> path = new ArrayList<>();
        parent[x] = -1;
        while (y != -1) {
            path.add(y);
            y = parent[y];
        }
        int size = path.size();
        if (size % 2 == 0) {
            result.add(path.get(size / 2 - 1));
        }
        result.add(path.get(size / 2));
        return result;
    }
    public int findLongNode(int n, int[] parent, List<Integer>[] adj) {
        int len = adj.length;
        Queue<Integer> queue = new ArrayDeque<>();
        boolean[] visit = new boolean[len];
        queue.offer(n);
        visit[n] = true;
        int node = -1;
        while (!queue.isEmpty()) {
            node = queue.poll();
            for (int ad : adj[node]) {
                if (!visit[ad]) {
                    visit[ad] = true;
                    parent[ad] = node;
                    queue.offer(ad);
                }
            }
        }
        return node;
    }
}
```



# 318.最大单词长度乘积

```java
class Solution {
    public int maxProduct(String[] words) {
        int[] wordDig = new int[words.length];
        for (int i = 0; i < words.length; i++) {
            for (int j = 0; j < words[i].length(); j++) {
                wordDig[i] = wordDig[i] | (1 << (words[i].charAt(j) - 'a'));
            }
        }
        int result = 0;
        for (int i = 0; i < wordDig.length; i++) {
            for (int j = i + 1; j < wordDig.length; j++) {
                if ((wordDig[i] & wordDig[j]) == 0) {
                    result = Math.max(result, words[i].length() * words[j].length());
                }
            }
        }
        return result;
    }
}
```



# 322.零钱兑换

```java
class Solution {
    public int coinChange(int[] coins, int amount) {
        int[] dp = new int[amount + 1];
        Arrays.fill(dp, 99999999);
        int len = coins.length;
        dp[0] = 0;
        for (int i = 0; i < len; i++) {
            for (int j = coins[i]; j <= amount; j++) {
                dp[j] = Math.min(dp[j], dp[j - coins[i]] + 1);
            }
        }
        return dp[amount] == 99999999 ? -1 : dp[amount];
    }
}
```

# 324.摆动序列Ⅱ

~~~java
class Solution {
    public void wiggleSort(int[] nums) {
        int len = nums.length;
        int[] temp = new int[len];
        for (int i = 0; i < len; i++) {
            temp[i] = nums[i];
        }
        Arrays.sort(temp);
        int right = len - 1;
        for (int i = 1; i < len; i+=2) {
            nums[i] = temp[right--];
        }
        for (int i = 0; i < len; i+=2) {
            nums[i] = temp[right--];
        }
    }
}
~~~

# 326.3的幂

```java
class Solution {
    public boolean isPowerOfThree(int n) {
        if (n <= 0) {
            return false;
        }
        if (n == 1) {
            return true;
        }
        int number = 1;
        for (int i = 0; i < 32; i++) {
            number *= 3;
            if (number == n) {
                return true;
            }
        }
        return false;
    }
}
```

```java
class Solution {
    public boolean isPowerOfThree(int n) {
        while (n!=0 && (n%3 == 0)) {
            n /= 3;
        }
        return n == 1;
    }
}
```

# 328.奇偶链表

```java
/**
 * Definition for singly-linked list.
 * public class ListNode {
 *     int val;
 *     ListNode next;
 *     ListNode() {}
 *     ListNode(int val) { this.val = val; }
 *     ListNode(int val, ListNode next) { this.val = val; this.next = next; }
 * }
 */
class Solution {
    public ListNode oddEvenList(ListNode head) {
        if (head == null || head.next == null) {
            return head;
        }
        ListNode newHead1 = head;
        ListNode newHead2 = head.next;
        int index = 0;
        ListNode node = head.next.next;
        ListNode newNode1 = newHead1;
        ListNode newNode2 = newHead2;
        while (node != null) {
            ListNode tempNode = node;
            node = node.next;
            if (index++ % 2 == 0) {
                newNode1.next = tempNode;
                newNode1 = newNode1.next;
            } else {
                newNode2.next = tempNode;
                newNode2 = newNode2.next;
            }
        }
        newNode1.next = newHead2;
        newNode2.next = null;
        return newHead1;
    }
}
```

# 338.比特位计数

```java
class Solution {
    public int[] countBits(int n) {
        int[] result = new int[n + 1];
        result[0] = 0;
        int maxNumber = 0;
        for (int i = 1; i <= n; i++) {
            if (i == (maxNumber == 0 ? 1 : maxNumber) * 2) {
                maxNumber = i;
            }
            result[i] = result[i - maxNumber] + 1;
        }
        return result;
    }
}
```



# 342.4的幂

```java
class Solution {
    public boolean isPowerOfFour(int n) {
        while (n !=0 && (n % 4 == 0)) {
            n /= 4;
        }
        return n == 1;
    }
}
```

# 344.反转字符串

```java
class Solution {
    public void reverseString(char[] s) {
        int len = s.length;
        for (int i = 0; i < len / 2; i++) {
            char temp = s[i];
            s[i] = s[len - 1 - i];
            s[len - 1 - i] = temp;
        }
    }
}
```

# 345.反转字符串中的元音字母

```java
class Solution {
    public String reverseVowels(String s) {
        Set<Character> resultSet = new HashSet<>();
        resultSet.add('a');
        resultSet.add('e');
        resultSet.add('i');
        resultSet.add('o');
        resultSet.add('u');
        resultSet.add('A');
        resultSet.add('E');
        resultSet.add('I');
        resultSet.add('O');
        resultSet.add('U');
        char[] sChar = s.toCharArray();
        Deque<Integer> indexQueue = new LinkedList<>();
        Deque<Character> charQueue = new LinkedList<>();
        for (int i = 0; i < sChar.length; i++) {
            if (resultSet.contains(sChar[i])) {
                indexQueue.addFirst(i);
                charQueue.addFirst(sChar[i]);
            }
        }
        while (!indexQueue.isEmpty() && !charQueue.isEmpty()) {
            int index = indexQueue.removeLast();
            char cChar = charQueue.removeFirst();
            sChar[index] = cChar;
        }
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < sChar.length; i++) {
            sb.append(sChar[i]);
        }
        return sb.toString();
    }
}
```



# 347.前K个高频元素

```java
class Solution {
    public int[] topKFrequent(int[] nums, int k) {
        Map<Integer, Integer> map = new HashMap<>();
        for (int num : nums) {
            map.put(num, map.getOrDefault(num, 0) + 1);
        }
        PriorityQueue<Integer> pQueue = new PriorityQueue<>(new Comparator<Integer>() {
            @Override
            public int compare(Integer n1, Integer n2) {
                return map.get(n1) - map.get(n2);
            }
        });
        for (Integer key : map.keySet()) {
            pQueue.offer(key);
            while (pQueue.size() > k) {
                pQueue.poll();
            }
        }
        int[] result = new int[k];
        for (int i = 0; i < k; i++) {
            result[i] = pQueue.poll();
        }
        return result;
    }
}
```

# 349.两个数组的交集

```java
class Solution {
    public int[] intersection(int[] nums1, int[] nums2) {
        Set<Integer> resultSet = new HashSet<>();
        for (int i = 0; i < nums1.length; i++) {
            resultSet.add(nums1[i]);
        }
        Set<Integer> distinctSet = new HashSet<>();
        for (int i = 0; i < nums2.length; i++) {
            if (resultSet.contains(nums2[i])) {
                distinctSet.add(nums2[i]);
            }
        }
        int[] result = new int[distinctSet.size()];
        int index = 0;
        for (Integer temp : distinctSet) {
            result[index++] = temp;
        }
        return result;
    }
}
```

# 350.两个数组的交集Ⅱ

```java
class Solution {
    public int[] intersect(int[] nums1, int[] nums2) {
        Map<Integer, Integer> n1Map = new HashMap<>();
        List<Integer> resultList = new ArrayList<>();
        for (int i = 0; i < nums1.length; i++) {
            n1Map.put(nums1[i], n1Map.getOrDefault(nums1[i], 0) + 1);
        }
        for (int i = 0; i < nums2.length; i++) {
            int count = n1Map.getOrDefault(nums2[i], 0);
            if (count > 0) {
                resultList.add(nums2[i]);
                n1Map.put(nums2[i], count - 1);
            }
        }
        int[] result = new int[resultList.size()];
        for (int i = 0; i < result.length; i++) {
            result[i] = resultList.get(i);
        }
        return result;
    }
}
```



# 367.有效的完全平方数

```java
class Solution {
    public boolean isPerfectSquare(int num) {
        return Math.pow((int)Math.pow(num, 0.5), 2) == num;
    }
}
```

```java
class Solution {
    public boolean isPerfectSquare(int num) {
        int number = binarySearch(num, 0, num+1) - 1;
        return number * number == num;
    }
    public int binarySearch(int n, int left, int right) {
        while (left + 1 < right) {
            int mid = (left + right) / 2;
            if ((long)mid * (long)mid > (long)n) {
                right = mid;
            } else {
                left = mid;
            }
        }
        return right;
    }
}
```

# 371.两整数之和

```java
class Solution {
    public int getSum(int a, int b) {
        return a+b;
    }
}
```

```java
class Solution {
    public int getSum(int a, int b) {
        while (b != 0) {
            int temp = (a & b) << 1;
            a = a ^ b;
            b = temp;
        }
        return a;
    }
}
```

# 373.查找和最小的K对数字

```java
class Solution {
    public List<List<Integer>> kSmallestPairs(int[] nums1, int[] nums2, int k) {
        PriorityQueue<int[]> heap = new PriorityQueue<>((a, b) -> a[0] - b[0]);
        for (int i = 0; i < nums1.length; i++) {
            heap.add(new int[]{nums1[i] + nums2[0], i, 0});
        }
        List<List<Integer>> result = new ArrayList<>();
        while (k > 0) {
            int[] temp = heap.poll();
            List<Integer> tempList = new ArrayList<>();
            tempList.add(nums1[temp[1]]);
            tempList.add(nums2[temp[2]]);
            result.add(tempList);
            k--;
            if (temp[2] < nums2.length - 1) {
                heap.add(new int[]{nums1[temp[1]] + nums2[temp[2] + 1], temp[1], temp[2] + 1});
            }
        }
        return result;
    }
}
```



# 374.猜数字大小

```java
/** 
 * Forward declaration of guess API.
 * @param  num   your guess
 * @return 	     -1 if num is higher than the picked number
 *			      1 if num is lower than the picked number
 *               otherwise return 0
 * int guess(int num);
 */

public class Solution extends GuessGame {
    public int guessNumber(int n) {
        int left = 1;
        int right = n;
        while (left < right) {
            int mid = left + (right - left) / 2;
            int tempResult = guess(mid);
            if (tempResult == 0) {
                return mid;
            } else if (tempResult == -1) {
                right = mid;
            } else {
                left = mid + 1;
            }
        }
        return left;
    }
}
```

# 377.组合总和Ⅳ

```java
class Solution {
    public int combinationSum4(int[] nums, int target) {
        int[] dp = new int[target + 1];
        dp[0] = 1;
        for (int i = 1; i < dp.length; i++) {
            for (int temp : nums) {
                int need = i - temp;
                if (need >= 0) {
                    dp[i] += dp[need];
                }
            }
        }
        return dp[dp.length - 1];
    }
}
```



# 378.有序矩阵中第K小的元素

```java
class Solution {
    public int kthSmallest(int[][] matrix, int k) {
        PriorityQueue<Integer> queue = new PriorityQueue<>();
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix.length; j++) {
                queue.add(matrix[i][j]);
            }
        }
        while (k > 1) {
            queue.poll();
            k--;
        }
        return queue.peek();
    }
}
```



# 382.链表随机节点

```java
/**
 * Definition for singly-linked list.
 * public class ListNode {
 *     int val;
 *     ListNode next;
 *     ListNode() {}
 *     ListNode(int val) { this.val = val; }
 *     ListNode(int val, ListNode next) { this.val = val; this.next = next; }
 * }
 */
class Solution {

    int[] list = new int[10001];;
    int size;

    public Solution(ListNode head) {
        size = 0;
        while (head != null) {
            list[size++] = head.val;
            head = head.next;
        }
    }
    
    public int getRandom() {
        Random random = new Random();
        int randomNum = random.nextInt(size);
        return list[randomNum];
    }
}

/**
 * Your Solution object will be instantiated and called as such:
 * Solution obj = new Solution(head);
 * int param_1 = obj.getRandom();
 */
```

# 383.赎金信

```java
class Solution {
    public boolean canConstruct(String ransomNote, String magazine) {
        char[] magaChar = magazine.toCharArray();
        char[] ranChar = ransomNote.toCharArray();
        Map<Character, Integer> resultMap = new HashMap<>();
        for (int i = 0; i < magaChar.length; i++) {
            resultMap.put(magaChar[i], resultMap.getOrDefault(magaChar[i], 0) + 1);
        }
        for (int i = 0; i < ranChar.length; i++) {
            int tempCnt = resultMap.getOrDefault(ranChar[i], 0);
            if (tempCnt == 0) {
                return false;
            }
            resultMap.put(ranChar[i], tempCnt - 1);
        }
        return true;
    }
}
```



# 386.字典序排数

```java
class Solution {
    public List<Integer> lexicalOrder(int n) {
        List<Integer> result = new ArrayList<>();
        for (int i = 1; i <= n; i++) {
            result.add(i);
        }
        Collections.sort(result, (a, b) -> String.valueOf(a).compareTo(String.valueOf(b)));
        return result;
    }
}
```

# 387.字符串中的第一个唯一字符

```java
class Solution {
    public int firstUniqChar(String s) {
        int[] resultArray = new int[26];
        char[] sChar = s.toCharArray();
        for (int i = 0; i < resultArray.length; i++) {
            resultArray[i] = -99;
        }
        for (int i = 0; i < sChar.length; i++) {
            int index = sChar[i] - 'a';
            if (resultArray[index] == -99) {
                resultArray[index] = i;
            } else if (resultArray[index] >= 0) {
                resultArray[index] = -1;
            }
        }
        int min = Integer.MAX_VALUE;
        for (int i = 0; i < resultArray.length; i++) {
            if (resultArray[i] >= 0) {
                min = Math.min(min, resultArray[i]);
            }
        }
        return min == Integer.MAX_VALUE ? -1 : min;
    }
}
```



# 389.找不同

```java
class Solution {
    public char findTheDifference(String s, String t) {
        char[] sChar = s.toCharArray();
        char[] tChar = t.toCharArray();
        int sLen = sChar.length;
        int tLen = tChar.length;
        int[] sx = new int[26];
        for (int i = 0; i < sx.length; i++) {
            sx[i] = 0;
        }
        for (int i = 0; i < tLen; i++) {
            sx[tChar[i] - 'a']++;
        }
        for (int i = 0; i < sLen; i++) {
            sx[sChar[i] - 'a']--;
        }
        for (int i = 0; i < sx.length; i++) {
            if (sx[i] != 0) {
                return (char) (i + (int)'a');
            }
        }
        return '.';
    }
}
```

# 392.判断子序列

```java
class Solution {
    public boolean isSubsequence(String s, String t) {
        if (s.length() == 0) {
            return true;
        }
        char[] cs = s.toCharArray();
        char[] ct = t.toCharArray();
        int left = 0;
        for (int i = 0; i < ct.length; i++) {
            if (ct[i] == cs[left]) {
                left++;
            }
            if (left == cs.length) {
                return true;
            }
        }
        return false;
    }
}
```

# 397.整数替换

```java
class Solution {
    public int integerReplacement(int n) {
        return findMinNumber(n, 0);
    }
    public int findMinNumber(long n, int count) {
        if (n == 1) {
            return count;
        }
        count++;
        if (n % 2 == 1) {
            return Math.min(findMinNumber(n + 1, count), findMinNumber(n - 1, count));
        } else {
            return findMinNumber(n / 2, count);
        }
    }
}
```

### 
