...
public class Codec {
    ...
    public String encode(String longUrl) {
        String shortUrl = "http://tinyurl.com/" + resultMap.size();
...
class Solution {
    ...
    public int dfs(TreeNode node) {
        ...
        result = Math.max(leftLen + rightLen + 1, result);
...
class Solution {
    public int leastBricks(List<List<Integer>> wall) {
        ...
        for (int i = 0; i < wall.size(); i++) {
            List<Integer> thisWall = wall.get(i);
            int sum = 0;
            for (int j = 0; j < thisWall.size() - 1; j++) {
                int len = thisWall.get(j);
                sum += len;
                tempMap.put(sum, tempMap.getOrDefault(sum, 0) + 1);
            }
        }
...
class Solution {
    public int subarraySum(int[] nums, int k) {
        ...
        for (int i = 0; i < s.length; i++) {
            int target = s[i] - k;
            int cnt = tempMap.getOrDefault(target, 0);
            result += cnt;
            tempMap.put(s[i], tempMap.getOrDefault(s[i], 0) + 1);
        }
        return result;
    }
}
...
class Solution {
    ...
    public int pdHelper(TreeNode node) {
        if (node == null) {
            return 0;
        }
        int leftSum = pdHelper(node.left);
        int rightSum = pdHelper(node.right);
...
class Solution {
    ...
    public boolean isSubtree(TreeNode root, TreeNode subRoot) {
        String subStr = middleHelper(subRoot);
        nodeSet = new HashSet<>();
        middleHelper(root);
        return nodeSet.contains(subStr);
    }

    public String middleHelper(TreeNode node) {
        if (node == null) {
            return "";
        }
        String str = "(" + middleHelper(node.left) + ")L" + node.val + "," + "(" + middleHelper(node.right) + ")R";
        nodeSet.add(str);
        return str;
    }
}
