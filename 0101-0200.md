...
class Solution {
    ...
    public void doIt(TreeNode node, int level) {
        ...
        List<Integer> tempList = result.get(level);
...
class Solution {
    ...
    public TreeNode sortedListToBST(ListNode head) {
        ...
        TreeNode root = doHelper(0, tempList.size() - 1);
        return root;
    }
    public TreeNode doHelper(int left, int right) {
        if (left > right) {
            return null;
        }
        int mid = (left + right) / 2;
        TreeNode node = new TreeNode(tempList.get(mid));
        node.left = doHelper(left, mid - 1);
        node.right = doHelper(mid + 1, right);
        return node;
    }
}
```
...
class Solution {
    ...
    public void doHelper(TreeNode node, int sum) {
        if (node == null) {
            return;
        }
        tempList.add(node.val);
        sum += node.val;
        if (node.left == null && node.right == null && sum == totalTarget) {
...
class Solution {
    ...
    public void find(int x, int y) {
        if (ttBoard[x][y] != 'O' || visited[x][y]) {
            return;
        }
        if (x == 0 || x == ttBoard.length - 1 || y == 0 || y == ttBoard[0].length - 1) {
            tempFlag = true;
        }
        visited[x][y] = true;
        if (x > 0) {
        ...
        if (x < ttBoard.length - 1) {
            find(x + 1, y);
        }
        if (y < ttBoard[0].length - 1) {
            find(x, y + 1);
        }
    }
...
class Solution {
    ...
    public void doIt(int index, int pre) {
        String tempStr = golbal.substring(pre, index == golbal.length() ? index : index + 1);
        if (index == golbal.length()) {
            if (isHv(tempStr) && pre != index) {
    ...
    public int canCompleteCircuit(int[] gas, int[] cost) {
        ...
        for (int i = 0; i < gas.length; i++) {
            ...
            if (oil < minOil) {
                minOil = oil;
                minOilIndex = i + 1;
                minOilIndex %= gas.length;
            }
        }
        return oil < 0 ? -1 : minOilIndex;
    }
}
```
