    ...
    public String removeDuplicates(String s, int k) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (sb.length() > 0 && sb.charAt(sb.length() - 1) == temp) {
                tempStack.push(tempStack.pop() + 1);
            } else {
                tempStack.push(1);
            }
            sb.append(temp);
    ...
    public List<Integer> circularPermutation(int n, int start) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            for (int j = result.size() - 1; j >= 0; j--) {
                result.add((result.get(j) ^ start | temp) ^ start);
            }
        }
        return result;
    }
...
class Solution {
    public String minRemoveToMakeValid(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            char temp = s.charAt(i);
            if (!charStack.isEmpty() && temp == ')' && charStack.peek() == '(') {
                charStack.pop();
                indexStack.pop();
            } else if (temp == '(' || temp == ')') {
                charStack.push(temp);
...
class Solution {
    ...
    public void findLand(int x, int y) {
...
class Solution {
    public int countServers(int[][] grid) {
        ...
        for (int i = 0; i < grid.length; i++) {
            for (int j = 0; j < grid[0].length; j++) {
                if (grid[i][j] == 1 && (heng[i] > 1 || lie[j] > 1)) {
                    result++;
                }
            }
        }
...
class Solution {
    public List<List<Integer>> groupThePeople(int[] groupSizes) {
        ...
        for (int i = 0; i < groupSizes.length; i++) {
            List<Integer> tempList = tempMap.getOrDefault(groupSizes[i], null);
            if (tempList == null) {
        ...
        for (Integer key : tempMap.keySet()) {
            ...
            for (int i = 0; i < tempList.size(); i += key) {
                List<Integer> tempRes = new ArrayList<>();
                for (int j = i; j < i + key; j++) {
                    tempRes.add(tempList.get(j));
                }
...
class CombinationIterator {
    ...
    public CombinationIterator(String characters, int combinationLength) {
        tempChar = characters.toCharArray();
        ttLen = combinationLength;
        doIt(0, "");
    }

    public void doIt(int index, String result) {
        if (result.length() == ttLen) {
            heap.add(result);
            return;
        }
        ...
        doIt(index + 1, result);
        doIt(index + 1, result + tempChar[index]);
    }
...
class Solution {
    public int removeCoveredIntervals(int[][] intervals) {
        ...
        return intervals.length - tempRes;
    }
}
```
    ...
    public List<Integer> sequentialDigits(int low, int high) {
        ...
        for (int i = minLen; i <= maxLen; i++) {
            for (int j = 0; j < 9 - i + 1; j++){
                Integer tempInt = Integer.valueOf(tempStr.substring(j, j + i));
